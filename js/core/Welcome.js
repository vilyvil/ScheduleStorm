class Welcome {
  ...
  getUnis() {
    ...
    $.getJSON(this.baseURL + "unis", function (data) {
      ...
      loading.remove(function () {
        ...
        if (
          thisobj.unis[localUni] != undefined &&
          thisobj.unis[localUni]["terms"][localTerm] != undefined
        ) {
          // Hide the modal
          ...
          $("#MyUniversity")
            .hide()
            .html(
              thisobj.unis[thisobj.uni]["name"] + " <span class='caret'></span>"
            )
            .fadeIn("slow");
  ...
  populateUniDropdown(data) {
    ...
    for (var uni in data) {
      ...
      var uniHTML =
        '<li class="dropdown-items"><a uni="' + uni + '">' + uniobj["name"];

      if (uniobj["scraping"] == true) {
        uniHTML +=
          '<span class="label label-default" style="margin-left: 10px;">Updating</span>';
      }
      ...
      html.click(function () {
        ...
        $("#MyUniversity")
          .hide()
          .html(self.unis[self.uni]["name"] + " <span class='caret'></span>")
          .fadeIn("slow");
  ...
  populateUnis(unis) {
    ...
    list.append(
      "<br><a href='https://github.com/Step7750/ScheduleStorm#supported-universities'>Don't see your school? Tell Us!</a>"
    );
