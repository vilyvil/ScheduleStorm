class ClassList {
  ...
  createTermDropdown() {
    ...
    $("#termselect").html(
      window.unis[this.uni]["terms"][this.term] +
    ...
    for (var term in window.unis[this.uni]["terms"]) {
      var html = $(
        '<li><a term="' +
          term +
          '">' +
          window.unis[this.uni]["terms"][term] +
          "</a></li>"
      );
  ...
  createLocationDropdown() {
    ...
    for (var location in window.unis[self.uni]["locations"]) {
      ...
      // Create the HTML
      var html = $(
        '<li><a location="' + thislocation + '">' + thislocation + "</a></li>"
      );
      ...
      html.click(function () {
        ...
        if (newlocation != self.location) {
          ...
          $("#locationselect").html(
            newlocation + ' <img src="assets/arrow.png">'
          );
  ...
  updateLocation(newlocation) {
    ...
    $("#classdata").slideUp(function () {
      ...
      if (Object.keys(self.classdata).length > 0) {
        ...
        $("#classdata")
          .text("There are no courses with that location :(")
          .slideDown();
      }
    });
  }
  ...
  getClasses() {
    ...
    $("#classdata").fadeOut(function () {
      ...
      // Get the class data
      $.getJSON(
        self.baseURL + "unis/" + self.uni + "/" + self.term + "/all",
        function (data) {
  ...
  generateClassDetails(element, path) {
    ...
    var button = $(
      this.generateAccordionHTML(
        "Details",
        path + "\\description",
        "accordionDetailButton"
      )
    );
  ...
  generateRMPLink(rmpdata, teacher) {
    ...
    if (rmpdata["id"] == undefined) return text;
      ...
      return (
        "<a href='https://www.ratemyprofessors.com/ShowRatings.jsp?tid=" +
        rmpdata["id"] +
        "' target='_blank' class='rmplink' rmpteacher='" +
        teacher +
        "'>" +
        text +
        "</a>"
  ...
  generateRMPTooltipHTML(rmpdata) {
    var html =
      "<b style='font-weight: bold; font-size: 14px;'>Rate My Professors</b><br>";
  ...
  generateClasses(data, element, path, addButton) {
    ...
    var html = $(
      "<div class='accordiontableparent'><table class='table accordiontable'><tbody></tbody></table></div>"
    );
    ...
    for (var index = 0; index < orderedClasses.length; index++) {
      ...
      for (var teacher in thisclass["teachers"]) {
        ...
        if (addedTeachers.indexOf(thisclass["teachers"][teacher]) == -1) {
          ...
          if (this.rmpdata[teacher] != undefined) {
            teachers +=
              " " + this.generateRMPLink(this.rmpdata[teacher], teacher);
          }
      ...
      thishtml +=
        "<td style='width: 25%;'>" + addedTimes.join("<br>") + "</td>";
          ...
          if (val == "classes") {
            ...
            if (thisdata[val]["classes"] != undefined) {
              ...
              if (window.mycourses.hasCourse(coursecode)) {
                self.appendCourseRemoveBtn(
                  coursecode,
                  thiselement.find("label")
                );
              } else {
  ...
  appendCourseAddBtn(coursecode, element) {
    ...
    var addbutton = $(
      '<div class="addCourseButton" code="' + coursecode + '">+</div>'
    );
  ...
  appendCourseRemoveBtn(coursecode, element) {
    ...
    var removebtn = $(
      '<div class="removeCourseButton" code="' + coursecode + '">×</div>'
    );
  ...
  appendClassAddBtn(id, path, element) {
    ...
    var button = $(
      '<td><button class="btn btn-default" classid="' +
        id +
        '" path="' +
        path +
        '">&plus;</button></td>'
  ...
  appendClassRemoveBtn(id, path, element) {
    ...
    var button = $(
      '<td><button class="btn btn-default" id="removeClassBtn" classid="' +
        id +
        '" path="' +
        path +
        '">×</button></td>'
  ...
  updateRemovedClass(classid) {
    ...
    if (removedClass.length > 0) {
      this.appendClassAddBtn(
        classid,
        removedClass.attr("path"),
        removedClass.parent().parent()
      );
      ...
      for (var prop in thisclass) {
        ...
        if (thisclass[prop].constructor === Array) {
          for (var newprop in thisclass[prop]) {
            if (
              thisclass[prop][newprop].toString().toLowerCase().indexOf(text) >
              -1
            ) {
              return true;
            }
          ...
          thisclass[prop].toString().toLowerCase().indexOf(text) > -1
        ) {
          return true;
        }
      }
      ...
      for (var key in data) {
        if (key != "description") {
          ...
          if (searchkey.toLowerCase().indexOf(text) > -1) {
            ...
            if (
              data[key]["description"] != undefined &&
              data[key]["description"]["name"] != undefined
            ) {
              if (
                data[key]["description"]["name"].toLowerCase().indexOf(text) >
                -1
              ) {
  ...
  generateAccordionHTML(value, path, customclasses) {
    if (customclasses)
      return (
        '<li class="has-children"><label path="' +
        path +
        '" accordopen="false" class="' +
        customclasses +
        '">' +
        value +
        "</label><ul></ul></li>"
